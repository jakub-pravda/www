...
func main() {
	...
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		for _, projectName := range projects {
			...
			deployProject(ctx, projectConfig, logsBucket)
		}
...
func deployProject(ctx *pulumi.Context, project staticSiteProject, logsBucket *s3.Bucket) {
	...
	if len(domains) > 0 {
		cdn := instantiateCloudfront(ctx, contentBucket, logsBucket, domains, project.indexDoc)
...
func instantiateCloudfront(ctx *pulumi.Context, contentBucket *s3.Bucket, logsBucket *s3.Bucket, domains []string, indexDoc string) *cloudfront.Distribution {
	mainDomain := domains[0]
